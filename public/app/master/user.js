...
const modalUser = (tipe='show') => $('#modal').modal(tipe);
...
function aktif(id) {
    ...
    let status_aktif = $("#active_"+id).is(':checked');

    let simpandata = $.ajax(url+'/aktif', {
        dataType: 'json',
        type: 'POST',
        data: {
            ID: id,
            aktif: (status_aktif) ? '1': '0'
        },
    })
...
function edit(id) {
    ...
    let data_user = $('#edit_'+id).attr('data-user');
